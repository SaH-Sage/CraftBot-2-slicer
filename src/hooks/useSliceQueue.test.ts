...
describe('parent-relative cascade (support pillars following their parent)', () => {
  const identity = { scale: [1, 1, 1], rotation: [0, 0, 0], mirror: [1, 1, 1], offset: null } as unknown as {
  ...
  it("re-derives a child's offset from the parent's newly-applied transform", () => {
    const items = [
      { id: 'main', status: 'ready', transform: { ...identity, offset: [0, 0] } },
      { id: 'pillar', status: 'ready', parentId: 'main', relativeOffset: [20, 0, 5], transform: { ...identity, offset: [20, 0] } },
    ] as unknown as typeof state.items
    const next = sliceQueueReducer(
      { ...state, items },
      { type: 'APPLY_TRANSFORMS', updates: [{ id: 'main', transform: { ...identity, offset: [100, 50] } }] },
    )
    // Parent moved by (+100, +50) with no rotation change — the pillar,
    // sitting 20mm "east" of the parent, should move by exactly the same delta.
    expect(next.items[1].transform?.offset).toEqual([120, 50])
  })

  it('marks a previously-sliced child stale when the cascade actually moves it', () => {
    const items = [
      { id: 'main', status: 'ready', transform: { ...identity, offset: [0, 0] } },
      {
        id: 'pillar',
        ...
        transform: { ...identity, offset: [20, 0] },
      },
    ] as unknown as typeof state.items
    const next = sliceQueueReducer(
      { ...state, items },
      { type: 'APPLY_TRANSFORMS', updates: [{ id: 'main', transform: { ...identity, offset: [40, 0] } }] },
    )
    expect(next.items[1].stale).toBe(true)
  })

  it('propagates through a two-level chain in a single dispatch (a pillar clicked onto another pillar)', () => {
    const items = [
      { id: 'main', status: 'ready', transform: { ...identity, offset: [0, 0] } },
      { id: 'pillarA', status: 'ready', parentId: 'main', relativeOffset: [20, 0, 5], transform: { ...identity, offset: [20, 0] } },
      // pillarB sits 3mm further out from pillarA, which itself sits 20mm out from main.
      { id: 'pillarB', status: 'ready', parentId: 'pillarA', relativeOffset: [3, 0, 2], transform: { ...identity, offset: [23, 0] } },
    ] as unknown as typeof state.items
    const next = sliceQueueReducer(
      { ...state, items },
      { type: 'APPLY_TRANSFORMS', updates: [{ id: 'main', transform: { ...identity, offset: [100, 0] } }] },
    )
    expect(next.items[1].transform?.offset).toEqual([120, 0]) // pillarA follows main
    expect(next.items[2].transform?.offset).toEqual([123, 0]) // pillarB follows pillarA's NEW position, same dispatch
  })

  it('leaves an orphaned child (parent no longer present) untouched rather than crashing', () => {
    const items = [
      { id: 'pillar', status: 'ready', parentId: 'gone', relativeOffset: [20, 0, 5], transform: { ...identity, offset: [20, 0] } },
      { id: 'other', status: 'ready', transform: { ...identity, offset: [0, 0] } },
    ] as unknown as typeof state.items
    const next = sliceQueueReducer(
      { ...state, items },
      { type: 'APPLY_TRANSFORMS', updates: [{ id: 'other', transform: { ...identity, offset: [50, 50] } }] },
    )
    expect(next.items[0].transform?.offset).toEqual([20, 0])
  })

  it('does not touch a child that has no relativeOffset (independent items keep their own position)', () => {
    const items = [
      { id: 'main', status: 'ready', transform: { ...identity, offset: [0, 0] } },
      { id: 'sibling', status: 'ready', parentId: 'main', transform: { ...identity, offset: [77, 77] } },
    ] as unknown as typeof state.items
    const next = sliceQueueReducer(
      { ...state, items },
      { type: 'APPLY_TRANSFORMS', updates: [{ id: 'main', transform: { ...identity, offset: [100, 50] } }] },
    )
    expect(next.items[1].transform?.offset).toEqual([77, 77])
  })
})
